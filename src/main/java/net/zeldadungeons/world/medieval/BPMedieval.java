...
public class BPMedieval extends BiomeProvider
{
    ...
    protected BPMedieval()
    {
        biomeCache = new BiomeCache(this);
        biomesToSpawnIn = new ArrayList<>();
    }

    public BPMedieval(long seed, WorldType type)
    {
        this();
        GenLayer[] genLayers = GLMedieval.initGenLayers(seed);
        this.unzoomedBiomes = genLayers[0];
        this.zoomedBiomes = genLayers[1];
    }

    public BPMedieval(World world)
    {
    ...
    {
        return this.biomeCache.getBiome(pos.getX(), pos.getZ(), Generizer.MEDIEVAL_HILLS);
    }
    ...
    public Biome[] getBiomesForGeneration(Biome[] biomes, int x, int z, int length, int width)
    {
        ...
        for (int i = 0; i < length * width; ++i)
        {
            ...
            {
                biomes[i] = Generizer.MEDIEVAL_HILLS;
            }
        }
    ...
    public Biome[] getBiomes(@Nullable Biome[] listToReuse, int x, int z, int width, int length, boolean cacheFlag)
    {
        ...
        for (int i = 0; i < width * length; ++i)
        {
            ...
            {
                listToReuse[i] = Generizer.MEDIEVAL_HILLS;
            }
        }
